...
export const SET_SELECTED_ORGANIZATION = '[USER] SET SELECTED ORGANIZATION';
...
function updateUserData(user) {
    ...
    switch (user.from) {
        ...
        case 'keycloak':
            {
                keycloakService.updateUserData(user)
                    .then(() => {
                        store.dispatch(Actions.showMessage({ message: "User data saved with keycloak" }));
                    })
                    .catch(error => {
                        store.dispatch(Actions.showMessage({ message: error.message }));
                    });
